...
helper.describeIntegration(helper.toSuiteName(__filename), function T() {
	...
	it(path.basename(__filename), (done) => {
		...
		var routeid = ""
		var sd10_optimization_problem_id = ""
		...
		const newaddress = [
			{ 
				"address" : "118 Bill Johnson Rd NE Milledgeville GA 31061",
				"lat" : 33.141784667969,
				"lng" : -83.237518310547,
				"time" : 0, 
				"sequence_no": 4
		...
		route4me.Routes.routeexamples_optiomization(param, (err, sdroute10stopsresult) => {
			...
			sd10_optimization_problem_id = sdroute10stopsresult.optimization_problem_id

			route4me.Routes.linkAddress(routeid, newaddress, options, (err, addroutedestination) => {
				debug("error  ", err)
				debug("result ", addroutedestination)
				expect(err).is.null
				expect(addroutedestination).exist
				if (addroutedestination == null || Object.keys(addroutedestination).length < 1)
				{
					console.log("Cannot insert the test destination.")
					route4me.Optimizations.remove(sd10_optimization_problem_id, (err, removeoptimization) => {
						debug("error  ", err)
						debug("result ", removeoptimization)
						expect(err).is.null
						expect(removeoptimization).exist
					})
					return
				}
				else
