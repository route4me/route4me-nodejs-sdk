...
describe(helper.toSuiteName(__filename), () => {
	describe("SDK methods", () => {
		...
		describe("get the configuration", () => {
			it("should call route4me", (done) => {
				resource.getMasterRouteAPIConfig((err, res) => {
					expect(err).is.null
					expect(res).to.exist
	
					helper.expectRequest(req,
						"GET",
						route4meClient.baseUrl5() + "/api/v5.0/recurring-routes/master-routes/apiconf",
						{},
						null
					)
					done()
		...
		describe("search for the routes", () => {
			const options = { order_by: [["route_name", "asc"]] };

			it("should call route4me with options", (done) => {
				resource.getMasterRouteDataTable(options, (err, res) => {
					expect(err).is.null
					expect(res).to.exist
	
					helper.expectRequest(req,
						"GET",
						route4meClient.baseUrl5() + "/api/v5.0/recurring-routes/master-routes/datatable",
						{
			...
			it("should call route4me without options", (done) => {
				resource.getMasterRouteDataTable((err, res) => {
					expect(err).is.null
					expect(res).to.exist
		...
		describe("schedule preview", () => {
			const data = {
				date_from_string: "2023-01-01",
				date_to_string: "2023-01-31"
			};
			
			it("should call route4me", (done) => {
				resource.schedulePreview(data, (err, res) => {
					expect(err).is.null
					expect(res).to.exist
	
					helper.expectRequest(req,
						"POST",
						route4meClient.baseUrl5() + "/api/v5.0/recurring-routes/schedule-preview",
						null,
						{
							date_from_string: "2023-01-01",
							date_to_string: "2023-01-31"
