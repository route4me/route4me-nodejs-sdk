...
class Schedules {
	 ...
	 * @param {module:route4me-node~RequestCallback}       [callback]
	 */
	getMasterRouteAPIConfig(callback) {
		return this.r._makeRequest5({
			method: "GET",
			path: "/api/v5.0/recurring-routes/master-routes/apiconf",
			validationContext: "SchedulesStatus.inline_response_200",
		}, callback)
	}
	 ...
	 * e.g., [["route_name", "asc"], ["name", "desc"]]
	 * @param {number}   [options.page = 1]               - Requested page.
	 * @param {number}   [options.per_page = 15]          - Number of File information per page.
	 * @param {string[]} [options.vehicles]               - Filter by vehicles
	 * @param {number[]} [options.members]                - Filter by members
	 * @param {string}   [options.start]                  - Start date as YYYY-MM-DDThh:mm:ss.sss+z
	 * @param {string}   [options.end]                    - End date as YYYY-MM-DDThh:mm:ss.sss+z
	 * @param {module:route4me-node~RequestCallback}       [callback]
	 */
	getMasterRouteDataTable(options, callback) {
		let opt = options || {}
		let cb = callback
		...
		return this.r._makeRequest5({
			method: "GET",
			path: "/api/v5.0/recurring-routes/master-routes/datatable",
			qs: opt,
			validationContext: "SchedulesStatus.MasterRoutes",
		}, cb)
	}
	 ...
	 *
	 * @param {object}   param
	 * @param {string}   param.date_from_string                       - Route date as YYYY-MM-DD.
	 * @param {string}   param.date_to_string                         - Route date as YYYY-MM-DD.
	 * @param {object[]} param.objects
	 * @param {string}   param.objects.id
	 * @param {object[]} param.objects.schedule
	 * @param {boolean}  param.objects.schedule.enabled
	 * @param {string}   param.objects.schedule.mode                  - daily
	 * @param {string}   param.objects.schedule.from
	 * @param {string}   param.objects.schedule.to
	 * @param {object}   param.objects.schedule.daily
	 * @param {object}   param.objects.schedule.monthly
	 * @param {object}   param.objects.schedule.annually
	 * @param {object[]} param.objects.schedule_blacklist
	 * @param {boolean}  param.objects.schedule_blacklist.enabled
	 * @param {string}   param.objects.schedule_blacklist.start
	 * @param {string}   param.objects.schedule_blacklist.end
	 * @param {module:route4me-node~RequestCallback}                    [callback]
	 */
	schedulePreview(param, callback) {
		return this.r._makeRequest5({
			method: "POST",
			path: "/api/v5.0/recurring-routes/schedule-preview",
			body: param,
			validationContext: "SchedulesStatus.inline_response_200_1",
		}, callback)
	}
}
